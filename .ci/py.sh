...
# pyenv install 3.8
if [ "$flag" -eq 1 ]; then
    command -v pyenv >/dev/null 2>&1 && pyenv install 3.8 && pyenv local 3.8 && { echo >&2 "Install python 3.8 using pyenv successfully.";} || { echo >&2 "Pyenv is not installed. Did not install python 3.8 using pyenv. Please install pyenv using 'bash .ci/pyenv-install.sh' and retry, or install python>=3.8 manually and retry."; exit 1;}
fi
